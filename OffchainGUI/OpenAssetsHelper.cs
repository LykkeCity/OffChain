...
namespace OffchainGUI
{
    ...
    public static class OpenAssetsHelper
    {
        ...
        public static async Task<Tuple<ColoredCoin[], Coin[]>> GetColoredUnColoredCoins(UniversalUnspentOutput[] walletOutputs,
            string assetId, Network network, string username, string password, string ipAddress, int port)
        {
            ...
            var walletColoredTransactions = await GetTransactionsHex(walletAssetOutputs, network, username, password, ipAddress, port);
            var walletUncoloredTransactions = await GetTransactionsHex(walletUncoloredOutputs, network, username, password, ipAddress, port);
            ...
            string username, string password, string ipAddress, int port)
        {
            ...
            {
                var ret = await GetTransactionHex(outputList[i].GetTransactionHash(), network, username, password, ipAddress, port);
                if (!ret.Item1)
                {
